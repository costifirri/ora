...
export default function Legami({ app }) {
  const { p, s, setS, setP, day, patchDay, markDone, flash, gentle, sendText } = app
  ...
  return (
    <div className="screen">
      <div style={{ padding: '4px 0 16px' }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, lineHeight: 1.1, margin: 0 }}>Legami</h1>
        <div className="meta" style={{ marginTop: 4 }}>
          {gentle
            ? 'Poche persone, conversazioni più vere'
            : p.convoLog.length === 1 ? '1 conversazione registrata' : `${p.convoLog.length} conversazioni registrate`}
        </div>
      </div>
      ...
      <div className="stack">
                    ...
                    <div style={{ padding: '2px 0 14px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                        ...
                        <button
                          ...
                          onClick={() => { setS({ openPerson: null }); flash('Te lo ricordo domani alle 18, senza insistere.') }}
                        >
                          Più tardi
                        </button>
                      </div>
        ...
        <div className="card sand">
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 24, padding: '22px 20px', minHeight: 132, display: 'flex', alignItems: 'center' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 20, lineHeight: 1.3 }}>
              {QUESTIONS[p.qIdx % QUESTIONS.length]}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
            <button className="btn-outline" style={{ flex: 1, minHeight: 48 }} onClick={() => setP(prev => ({ qIdx: prev.qIdx + 1 }))}>
              Un’altra
            </button>
            <button
              style={{ flex: 1, minHeight: 48, border: 0, borderRadius: 999, background: 'var(--sage-500)', color: 'var(--surface)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
              onMouseEnter={e => (e.currentTarget.style.background = 'var(--sage-600)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'var(--sage-500)')}
              onClick={() => flash('Tenuta da parte. La trovi in cima ai Legami quando ti serve.')}
            >
              Tienila per oggi
            </button>
          </div>
        ...
        <div className="card surface">
          <div className="h-card" style={{ marginBottom: 10 }}>Cosa hai notato</div>
          <div style={{ fontSize: 14, color: 'rgba(32,30,29,.65)', lineHeight: 1.55 }}>
            Nelle conversazioni in cui hai fatto almeno una domanda personale, il check-in successivo è stato calmo cinque
            volte su sei. Parlare meno ti calma più di quanto pensi.
          </div>
        </div>
      </div>
    </div>
